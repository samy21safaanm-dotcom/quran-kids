  ...
  if (completed) {
    ...
    return (
      ...
      >
        ...
        <div className="text-2xl font-black text-purple-900">أحسنت! أكملت التقييم 🎉</div>
        ...
        <div className="flex gap-1 mt-2">
          {scores.map((s, i) => (
            <motion.span
              ...
              className={`text-3xl ${s >= 0.9 ? 'text-yellow-400' : s >= 0.7 ? 'text-orange-400' : 'text-gray-400'}`}
            >
              ⭐
            </motion.span>
          ))}
      ...
      {!recognizedText ? (
        ...
        <>
          ...
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            className="w-full bg-white/90 rounded-3xl p-8 border-2 border-blue-300"
          >
            <div className="text-sm text-blue-500 font-bold mb-3">
              النص المسجل:
            </div>
            <div
              className="text-2xl md:text-3xl font-black text-blue-900 leading-relaxed mb-4"
              style={{ fontFamily: 'Tajawal, Arial' }}
            >
              {recognizedText}
            </div>
            ...
            <div className="text-center mb-4">
              {similarity >= 0.9 && <div className="text-xl font-black text-green-600">🎉 ممتاز جداً!</div>}
              {similarity >= 0.7 && similarity < 0.9 && <div className="text-xl font-black text-yellow-600">👏 جيد، حاول أكثر</div>}
              {similarity < 0.7 && <div className="text-xl font-black text-red-600">💪 حاول مرة أخرى</div>}
            </div>
          </motion.div>
          ...
          <div className="flex gap-3 flex-wrap justify-center">
            <motion.button
              ...
              className="px-6 py-3 rounded-full bg-blue-500 text-white font-bold shadow hover:bg-blue-600 transition"
            >
              🔄 سجل مجدداً
            </motion.button>
            <motion.button
              whileTap={{ scale: 0.9 }}
              onClick={handleNext}
              className="px-6 py-3 rounded-full bg-green-500 text-white font-bold shadow hover:bg-green-600 transition"
            >
              ✓ التالي
            </motion.button>
          </div>

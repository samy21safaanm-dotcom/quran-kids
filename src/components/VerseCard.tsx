...
const VerseCard: React.FC<VerseCardProps> = ({ ayah, index, isSelected, isCorrect, onClick, disabled }) => {
  return (
    <button
      className={`relative w-full max-w-[min(100%,20rem)] px-4 sm:px-5 md:px-6 py-3.5 sm:py-4 md:py-5 mb-2 rounded-2xl font-bold text-base sm:text-lg md:text-2xl text-purple-900 shadow-lg border-2 transition-all duration-200 min-h-11
    ...
    >
      {/* زخرفة ذهبية جانبية */}
      <span className="absolute left-2 top-2 text-yellow-400 text-sm sm:text-base md:text-lg select-none">{index !== undefined ? index + 1 : ''}</span>
